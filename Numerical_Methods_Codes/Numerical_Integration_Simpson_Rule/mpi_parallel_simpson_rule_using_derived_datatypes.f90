...
program parallel_simpson_rule

implicit none
include "mpif.h"
	
	double precision :: a, b, integration_result, local_a, local_b, local_sum, h, exact_result
	...
	if (my_id .eq. 0) then
		write(*, 100) "The integration for the given function between limits", a, " and", b, " =", integration_result
                100 format (A, F10.5, A, F10.5, A, F15.9)
 		write(*, 101) "Error associated between the numerically obtained value and the exact value =", (integration_result &
        		& - exact_result)
                101 format (A, F15.9)
	end if
...
contains
	...
	subroutine simpson_rule(x_s, x_e, n, h, local_result)
	implicit none
	
		double precision :: x_s, x_e, h, local_result, x
		integer :: n, i
		
		local_result = (func(x_s) + func(x_e))
		do i = 1, n-1
     			x = x_s + i * h
       			if (mod(i, 2) .eq. 0) then
       		   		local_result = local_result + 2.0d0 * func(x)
       			else
        		  		local_result = local_result + 4.0d0 * func(x)
       			end if
		end do
		
		local_result = local_result * h / 3.0d0
	...
	implicit none
		...
		y = sin(x) / (2.0d0 * x**3)     ! given function to integrate
		
	end function func

end program parallel_simpson_rule
